      ...
      | None      ->
        if w.hunger > 0 then
          ...
          | Some pList  ->  
            ...
            | Some pspos ->
              let poorSoul = List.find (fun (x:moose) -> 
                                          match x.position with 
                                          | Some px -> pspos=px 
                                          | None -> false) this.board.moose
...
let main (args: string array) =
  ...
  let n     = int args.[1]  // Boardwidth 
  let e     = int args.[2]  // # Mooses
  let felg  = int args.[3]  // Moose reproduction length
  let u     = int args.[4]  // # Wolves
  let fulv  = int args.[5]  // Wolf reproduction length
  let s     = int args.[6]  // Hunger length
